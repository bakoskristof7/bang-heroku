import React from 'react'
import { useDispatch } from 'react-redux';
import { addPickedCard } from '../../actions/game';

const GeneralStoreModalCard = ({card, mobile, setShowModal, activePlayer}) => {

    const dispatch = useDispatch();

    const cardName = card.type + '-' + card.pattern + '-' + card.number + '.JPG';
    const image = require('../../img/cards/action/'+ cardName );

    return (
            <img 
            src={image} 
            alt="" 
            className={`px-1 w-20 sm:w-24 md:w-32 rounded-xl mx-2 ${!mobile ? 'hover:rotate-1 hover:translate-y-4 transition-all w-48' : ''}`}
            onClick={() => { 
                if (activePlayer.user === localStorage.getItem('auth_username') || activePlayer.isRobot) {
                    dispatch(addPickedCard(card, activePlayer.user));
                    setShowModal(false);
                }
            }}
            />  
    )
}

export default GeneralStoreModalCard