...
const RoleModal = ({setShowModal, showModal, mobile}) => {
              ...
              <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-orange-100 outline-none focus:outline-none">
                ...
                <div className="flex items-start justify-between p-5 border-b border-solid border-slate-200 rounded-t">
                  <h3 className="text-3xl font-semibold">
                    {game.activePlayer.user} választ szerepet!
                  </h3>
                </div>
                ...
                <div className="relative p-6 flex-auto">
                  <div className='row-span-2 grid grid-flow-col-dense items-center m-auto'>
                    {game.roles.map((role, index) => <RoleCard role={role} mobile={false} setShowModal={setShowModal} activePlayer={game.activePlayer} key={index}/>)}
                  </div>
                </div>
